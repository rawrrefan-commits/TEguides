...
async function googleAccessToken(env) {
  ...
  if (!email || !privateKey) {
    throw new Error(
      "Firebase service account secret belum dikonfigurasi."
    );
  }

  const enc = o =>
    b64url(
      utf8(JSON.stringify(o))
    );
...
async function firestoreDoc(env, key) {
  ...
  const url =
    ...
    `/databases/(default)` +
    `/documents/keys/${encodeURIComponent(
      key
    )}`;
...
async function bindDevice(
  ...
  const url =
    ...
    `&currentDocument.updateTime=${encodeURIComponent(
      updateTime
    )}`;
...
async function validateKey(
  ...
  if (
    ...
    return {
      ...
      status: 403,
      message:
        "Key sudah kedaluwarsa (expired)."
    };
  }

  /*
   * Jika Firestore sudah memiliki deviceId,
   * hanya perangkat tersebut yang boleh memakai key.
   */
  if (saved) {
    if (saved !== deviceId) {
      return {
        ...
        status: 403,
        message:
          "Key ini sudah digunakan di perangkat lain."
      };
    }
  }

  /*
   * Jika deviceId belum ada,
   * perangkat pertama yang berhasil login
   * akan diikat ke key.
   */
  else if (bind) {
    const ok =
...
async function requireSession(
  ...
  /*
   * Session tidak dipercaya sendirian.
   * Key tetap dicek ulang ke Firestore.
   */
  const state =
    await validateKey(
      env,
    ...
    return json(
      {
        ok: false,
        message:
          "Server mengalami error. Cek konfigurasi Cloudflare/Firebase."
      },
      500
    );
  }
